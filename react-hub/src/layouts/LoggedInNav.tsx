...
export default function LoggedInNav({ children }: Props) {
  ...
  React.useEffect(() => {
    // use a wrapper like thing to put all the async logic bc useEffect
    // https://devtrium.com/posts/async-functions-useeffect
    const validate = async () => {
      const resp = await userRequests.get("/");
      console.log(resp);
  ...
  return (
    <AppNavBase
      children={children}
      AppbarButtons={({ handleDrawerOpen }) => (
        <>
          <Box sx={{ flexGrow: 1 }}>
            <IconButton
              color="inherit"
          ...
          <Box sx={{ flexGrow: 1 }}>
            <Typography variant="h6">Student Hub</Typography>
          </Box>
